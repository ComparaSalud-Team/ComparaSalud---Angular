...
interface FilaPrecio {
  especialidad: string;
  porClinica: Record<number, number | null>; // clinicId -> precio (null si no tiene)
}

// "Comparar proveedores" (tarjeta del dashboard). Compara clínicas/hospitales
// lado a lado: rating, especialistas, servicios/facilidades y precios por
// especialidad. Los precios salen de ClinicSpecialtyPrice
// (GET /api/clinics/{id}/pricing); si una clínica no tiene precios cargados
// para una especialidad, se muestra "—".
@Component({
...
export class CompararProveedoresComponent implements OnInit {
  ...
  verDisponibilidad(clinica: Clinic): void {
    // No hay todavía una forma de filtrar la búsqueda por clínica exacta;
    // se acerca al paciente a los proveedores de esa zona para que elija.
