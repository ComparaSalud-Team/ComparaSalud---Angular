...
export class MisCitasComponent implements OnInit {
  ...
  cargarCitas() {
    ...
    forkJoin({
      // El backend responde 404 cuando el paciente aún no tiene historial;
      // eso es un estado normal (no un error) y se traduce en lista vacía.
      // Cualquier otro código de error (500, red caída, etc.) SÍ debe
      // propagarse para que el usuario se entere de que algo falló de verdad.
      historial: this.citasService
        .obtenerHistorial(userId)
        .pipe(catchError((err) => (err?.status === 404 ? of([]) : throwError(() => err)))),
      proximas: this.citasService.obtenerProximas(userId),
    }).subscribe({
